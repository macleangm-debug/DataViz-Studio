...
export function ChartsPage() {
  ...
  const handleExportPdf = async (chartIds = null) => {
    ...
    try {
      ...
      // Logo and brand
      drawLogo(margin, 25, 16, [139, 92, 246]);
      pdf.setFont('helvetica', 'bold');
      pdf.setFontSize(24);
      pdf.setTextColor(30, 30, 30);
      pdf.text('DataViz Studio', margin + 22, 36);
      ...
      pdf.setFontSize(32);
      pdf.setTextColor(30, 30, 30);
      pdf.text('Charts Report', pageWidth / 2, 80, { align: 'center' });
      ...
      pdf.text(dateStr, pageWidth / 2, 92, { align: 'center' });
      
      // Summary box
      pdf.setFillColor(248, 250, 252); // Light gray
      pdf.roundedRect(margin, 110, contentWidth, 40, 3, 3, 'F');
      
      pdf.setFont('helvetica', 'bold');
      pdf.setFontSize(11);
      pdf.setTextColor(60, 60, 60);
      pdf.text('Report Summary', margin + 10, 122);
      
      pdf.setFont('helvetica', 'normal');
      pdf.setFontSize(10);
      pdf.setTextColor(80, 80, 80);
      pdf.text(`Total Charts: ${chartsToExport.length}`, margin + 10, 134);
      pdf.text(`Generated: ${dateStr}`, margin + 10, 144);
      
      // Chart types summary
      const chartTypes = [...new Set(chartsToExport.map(c => c.type))];
      pdf.text(`Chart Types: ${chartTypes.map(t => t.charAt(0).toUpperCase() + t.slice(1)).join(', ')}`, margin + 80, 134);
      
      // Charts list section
      pdf.setFont('helvetica', 'bold');
      pdf.setFontSize(14);
      pdf.setTextColor(30, 30, 30);
      pdf.text('Charts Included', margin, 175);
      
      pdf.setDrawColor(139, 92, 246);
      pdf.setLineWidth(2);
      pdf.line(margin, 179, margin + 30, 179);
      ...
      chartsToExport.forEach((chart, idx) => {
        if (yPos < pageHeight - 40) {
          ...
          pdf.text(`${chart.name}`, margin + 10, yPos);
          pdf.setTextColor(120, 120, 120);
          pdf.text(`(${chart.type.charAt(0).toUpperCase() + chart.type.slice(1)} Chart)`, margin + 10 + pdf.getTextWidth(chart.name) + 3, yPos);
          pdf.setTextColor(60, 60, 60);
          yPos += 10;
        }
      });
      ...
      // Footer on cover
      addFooter(1, chartsToExport.length + 1);
      ...
      for (let i = 0; i < chartsToExport.length; i++) {
        ...
        pdf.rect(0, 4, pageWidth, 25, 'F');
        
        pdf.setFont('helvetica', 'bold');
        pdf.setFontSize(18);
        pdf.setTextColor(30, 30, 30);
        pdf.text(chart.name, margin, 20);
        
        // Chart type badge
        pdf.setFillColor(139, 92, 246);
        const badgeText = `${chart.type.charAt(0).toUpperCase() + chart.type.slice(1)} Chart`;
        const badgeWidth = pdf.getTextWidth(badgeText) + 10;
        pdf.roundedRect(pageWidth - margin - badgeWidth, 12, badgeWidth, 8, 2, 2, 'F');
        pdf.setFont('helvetica', 'normal');
        pdf.setFontSize(9);
        pdf.setTextColor(255, 255, 255);
        pdf.text(badgeText, pageWidth - margin - badgeWidth + 5, 17.5);
        ...
        try {
          ...
          if (chartData.length > 0) {
            ...
            tempContainer.style.cssText = 'position: absolute; left: -9999px; width: 800px; height: 450px; background: #ffffff;';
            document.body.appendChild(tempContainer);
            
            // Generate chart options with light theme
            const options = {
              ...generateChartOptions(chart.type, chartData, chart.config || {}, chart.config?.theme || 'violet'),
              backgroundColor: '#ffffff'
            };
            // Override dark colors for PDF
            options.xAxis = options.xAxis ? { ...options.xAxis, axisLabel: { ...options.xAxis.axisLabel, color: '#666' } } : undefined;
            options.yAxis = options.yAxis ? { ...options.yAxis, axisLabel: { ...options.yAxis.axisLabel, color: '#666' }, splitLine: { lineStyle: { color: '#eee' } } } : undefined;
            ...
            pdf.rect(margin - 1, chartY - 1, imgWidth + 2, imgHeight + 2, 'S');
            ...
            // Data table section
            const tableY = chartY + imgHeight + 15;
            ...
            pdf.line(margin, tableY + 3, margin + 25, tableY + 3);
            
            // Table header
            const colWidth = contentWidth / 3;
            const headerY = tableY + 10;
            pdf.setFillColor(139, 92, 246);
            pdf.rect(margin, headerY, contentWidth, 8, 'F');
            
            pdf.setFont('helvetica', 'bold');
            pdf.setFontSize(9);
            pdf.setTextColor(255, 255, 255);
            pdf.text('Category', margin + 5, headerY + 5.5);
            pdf.text('Value', margin + colWidth + 5, headerY + 5.5);
            pdf.text('Share', margin + colWidth * 2 + 5, headerY + 5.5);
            ...
            chartData.slice(0, 8).forEach((d, idx) => {
              if (rowY < pageHeight - 30) {
                ...
                pdf.setTextColor(60, 60, 60);
                pdf.text(String(d.name || '').substring(0, 30), margin + 5, rowY + 5);
                pdf.text(d.value?.toLocaleString() || '0', margin + colWidth + 5, rowY + 5);
                
                // Share with color indicator
                pdf.setFillColor(139, 92, 246);
                pdf.rect(margin + colWidth * 2 + 5, rowY + 1.5, (parseFloat(share) / 100) * 30, 4, 'F');
                pdf.setTextColor(60, 60, 60);
                pdf.text(`${share}%`, margin + colWidth * 2 + 40, rowY + 5);
            ...
            // Table border
            pdf.setDrawColor(230, 230, 230);
            pdf.rect(margin, headerY, contentWidth, rowY - headerY, 'S');
            
          } else {
            pdf.setFontSize(12);
            pdf.setTextColor(150, 150, 150);
            pdf.text('No data available for this chart', pageWidth / 2, 100, { align: 'center' });
          }
        } catch (chartError) {
          console.error('Error rendering chart:', chartError);
          pdf.setFontSize(11);
          pdf.setTextColor(200, 100, 100);
          pdf.text('Unable to render chart', pageWidth / 2, 80, { align: 'center' });
        }
        
        // Footer
        addFooter(i + 2, chartsToExport.length + 1);
      }
